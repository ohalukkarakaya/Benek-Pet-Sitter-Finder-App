...
const getAfterEventContentListController = async ( req, res ) => {
    try{
        ...
        const skip = lastItemId !== 'null' ? meetingEvent.afterEvent.findIndex( afterEventObject => afterEventObject._id.toString() === lastItemId ) + 1 : 0;
        var searchedEvent = await Event.findById( eventId ).select( 'afterEvent' ).slice( 'afterEvent', [ skip, limit ] ).lean().exec();
        ...
        if(
            ...
            || (
                ...
                && !(
                    ...
                    || meetingEvent.eventOrganizers.includes( userId ) 
                    || meetingEvent.willJoin.includes( userId ) 
                    || meetingEvent.joined.includes( userId )
                )
            )
        ){
        ...
        const promises = searchedEvent.afterEvent.map(
            async ( afterEventObject ) => {
                ...
                const firstFiveUserLimit = Math.min(afterEventObject.likes.length, 5);
                                                
                const likedUsers = await Promise.all(
                    afterEventObject.likes.slice(0, firstFiveUserLimit).map(
                        async ( userId ) => {
