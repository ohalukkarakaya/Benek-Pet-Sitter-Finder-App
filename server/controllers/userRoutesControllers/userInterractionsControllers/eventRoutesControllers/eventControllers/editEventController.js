...
const editEventController = async (req, res) => {
    try{
        ...
        if( newEventDate ){
            if(
                Date.parse (newEventDate ) <= Date.now()
            ){
                return res.status( 400 )
                          .json(
                                {
            ...
            const parsedDate = Date.parse( newEventDate );
            meetingEvent.date = parsedDate
            meetingEvent.expiryDate = new Date( parsedDate + 7*24*60*60*1000 );
