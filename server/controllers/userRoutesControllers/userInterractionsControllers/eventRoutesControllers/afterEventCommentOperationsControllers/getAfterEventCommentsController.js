...
const getAfterEventCommentsController = async ( req, res ) => {
    try{
        ...
        let skip = lastItemId === 'null' ? afterEvent.comments.findIndex( comments => comments._id.toString() === lastItemId ) + 1 : 0;
        ...
        if( limit >= 0 ){
            const commentList = afterEvent.comments.reverse().filter(
                commentObject =>
                    commentObject.userId.toString() !== userId
            );
