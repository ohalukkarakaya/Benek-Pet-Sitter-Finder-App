...
const followPetController = async (req, res) => {
    try{
        ...
        const isAlreadyFollowedUserSide = user.followingUsersOrPets
                                              .find(
                                                    followingIdObject =>
                                                        followingIdObject.type === "pet"
                                                        && followingIdObject.followingId
                                                                            .toString() === petId
                                              );

        const isAlreadyFollowedPetSide = pet.followers
                                            .find(
                                                followerId =>
                                                    followerId.toString() === req.user
                                                                                 ._id
                                                                                 .toString()
                                            );

        if(
            !isAlreadyFollowedUserSide 
            && isAlreadyFollowedPetSide
        ...
        const isAlreadyFollowed = isAlreadyFollowedUserSide 
                                  && isAlreadyFollowedPetSide;
        ...
        return res.status( 200 )
                  .json(
                        {
                            error: false,
                            message: "pet followed or follow pet removed succesfully"
                        }
                  );
    }catch(err){
