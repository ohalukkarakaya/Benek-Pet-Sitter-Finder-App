...
const getPetImageCommentsController = async ( req, res ) => {
    try{
        ...
        let skip = image.comments.findIndex( comments => comments._id.toString() === lastItemId ) + 2;

        let comments = [];
        let usersComments = image.comments.filter(
            commentObject =>
                commentObject.userId.toString() === userId
        );

        if( skip === 0 || usersComments.length > skip ){
            if( usersComments.length > limit ){
                const startIndex = skip > 0 && usersComments.length > skip 
                                    ? skip - 1 : 0;

                const endIndex = ( startIndex + limit ) > ( usersComments.length -1 )
                                    ? usersComments.length -1 : startIndex + ( limit - 1 );
        ...
        if( limit >= 0 ){
            const commentList = image.comments.reverse().filter(
                commentObject =>
                    commentObject.userId.toString() !== userId
            );

            const startIndex = skip > 0 && commentList.length > skip
                                ? skip - 1 : 0;

            const endIndex = ( startIndex + limit ) > ( commentList.length -1 )
                                ? commentList.length - 1 : startIndex + ( limit - 1 );
        ...
        if( comments.length > 0 ){
            for( let commentObject of comments ){
                ...
                if( firstFiveOfLikeLimit > 0 ){
                    ...
                    const likeCount = commentObject.likes.length;
            ...
            return res.status( 200 ).json({
                ...
                totalCommentCount: image.comments.length,
