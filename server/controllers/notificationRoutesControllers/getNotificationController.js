...
const getNotificationController = async ( req, res ) => {
    try{
        ...
        if( lastItemId !== 'null' ){
            const lastItem = await Notification.findById(lastItemId);
            if(lastItem){
                notificationFilter.createdAt = { $gt: lastItem.createdAt };
            }
        }

        const totalNotificationCount= await Notification.countDocuments( notificationQuery );
        const notifications = await Notification.find( notificationFilter ).sort({ createdAt: 1 }).limit( limit ).lean();
        ...
        if( preparedNotificationData.error ){
