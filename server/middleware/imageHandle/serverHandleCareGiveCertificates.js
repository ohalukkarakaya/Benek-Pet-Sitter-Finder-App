...
const storage = multerS3(
    {
        ...
        key: (req, file, cb) => {
            try{
                ...
                const newFileName = `${userId}_careGiveCertificate${imageId}.${splitedOriginalName[ splitedOriginalName.length - 1 ]}`;
                req.careGiveCertificateNewFileName = newFileName;
                req.cdnUrl = `${ process.env.CDN_SUBDOMAIN }profileAssets/${ userId }/careGiveCertificates/${newFileName}`;
                
                cb( null, 
                    "profileAssets/"+userId+"/careGiveCertificates/"+newFileName
                );
...
const uploadCareGiverCertificate = async ( req, res, next ) => {
    ...
    export { uploadCareGiverCertificate };